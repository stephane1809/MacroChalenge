...
public class NutriRankClientPosts: ChallengePostRepositoryProtocol {
    ...
    public func fetchAllPosts() async -> Result<[NutriRankDomain.Post], Error> {
        do {
            let database = CKContainer(identifier: identifier).publicCloudDatabase
    ...
    public func getOwnersPost(_ member: NutriRankDomain.Member) async -> Result<[NutriRankDomain.Post], Error> {
        let database = CKContainer(identifier: identifier).publicCloudDatabase
        do {
